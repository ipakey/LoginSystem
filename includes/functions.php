...
function createUser($conn_tdc, $name, $email, $pwd, $date, $verified){

    $sql = "INSERT INTO `testlogin` (cu_name, cu_email, cu_pwd, cu_confirmed, cu_startDate) VALUES ($name, $email, $pwd, $verified, $date);";
    $stmt = mysqli_stmt_init($conn_tdc);
    if(!mysqli_stmt_prepare($stmt, $sql)){
        header("location: ../partPages/signUp.php?error=stmfailed");
        exit();
    ...
    mysqli_stmt_bind_param($stmt, "ssssd", $name, $email, $pwd, $verified, $date);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
